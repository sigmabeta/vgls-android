...
@Dao
interface ComposersForSongDao : JoinDao<ComposerEntity> {
    @Query(QUERY_JOIN)
    ...
    companion object {
        ...
        private const val ROW_JOIN_ID_ONE = "$TABLE_JOIN.${SongComposerJoin.ROW_FOREIGN_KEY_ONE}"
        private const val ROW_JOIN_ID_MANY =
            "$TABLE_JOIN.${SongComposerJoin.ROW_FOREIGN_KEY_TWO}"
