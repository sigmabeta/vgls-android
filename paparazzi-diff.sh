...
#
# Usage:
#   ./paparazzi-diff.sh [--keep] [--top N] [gradle-task]
#     gradle-task  defaults to :vgls:android:ui:previews:real:recordPaparazziDebug;
#                  pass a scoped task to narrow it if the screenshot layout grows.
#
set -u
...
TOP=5
NORECORD=0
TASK=":vgls:android:ui:previews:real:recordPaparazziDebug"
while [ $# -gt 0 ]; do
  case "$1" in
    --keep)      KEEP=1 ;;
...
if [ "$NORECORD" -eq 0 ]; then
  ...
  # Non-fatal on task failure: Paparazzi writes each golden as its test runs, so a stray
  # Layoutlib/threading crash on a single screen (e.g. a HandlerThread under Layoutlib) still leaves
  # the rest recorded — and the ranking below only looks at goldens that actually changed vs HEAD.
  if ! ./gradlew "$TASK" --console=plain -q; then
    echo "⚠ the record task reported failures (continuing — most goldens still recorded; ranking below"
    echo "  reflects whatever changed vs HEAD). If nothing ranks, the task failed before writing any."
  fi
else
