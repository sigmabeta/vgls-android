...
fun RemasterAppUi(
    ...
    // Short windows (e.g. landscape phone) collapse the top bar on scroll to reclaim vertical space.
    val windowInfo = LocalWindowInfo.current
    val heightDp = with(LocalDensity.current) { windowInfo.containerSize.height.toDp() }
    val actualModifier = if (heightDp < HEIGHT_COMPACT) {
...
fun AppContent(
    ...
    val displayWidthClass = syntheticWidthClass ?: currentWindowWidthClassSynthetic()
    val layoutType = navSuiteTypeFor(displayWidthClass)
